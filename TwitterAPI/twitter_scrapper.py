    ...
    def parse_tweets(items_html):
        ...
        for li in soup.find_all("li", class_='js-stream-item'):
            ...
            if text_p is not None:
                tweet['text'] = text_p.get_text().encode('utf-8').replace("\n", " ")
    ...
    def save_tweets(self, tweets):
        ...
        for tweet in tweets:
            if tweet['created_at'] is not None:
                ...
                fmt = "%Y-%m-%d %H:%M:%S"
                row = [long(tweet['tweet_id']),time.strftime(fmt),tweet['retweets'], tweet['favorites'],tweet['text'].replace('\n'," "),
                        long(tweet['user_id']),tweet['user_screen_name'].encode('utf8'),tweet['user_name'].encode('utf8')]
...
if __name__ == '__main__':
    with open(OUT_FILE, "w") as f:
        writer = csv.writer(f)
        writer.writerow(["Id", "CreatedAt","RetweetCount", "UsrFavouriteCount","Text", "User ID","User Screen Name","User Name"])
        twit = TwitterSearchImpl(0, 5, writer)
        twit.search("%23python")
